...
|  |  |
|--|--|
| <span class="notranslate"> `--enter-count`   </span> |enable limit for enters count (by default disabled), use only with "limit" |
| <span class="notranslate"> `--io-and-memory`   </span> |enable limit for IO and memory (by default disabled), use only with "limit" |
